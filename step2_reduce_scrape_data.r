...
remake_db <- F
make_db <- F
remake_visits_data <- F

tt <- timed()
### REDUCED DATASET: one row per server per week or per player per sever per week
### reset by week tables
if (make_db) {
  dbSendQuery(con, "DROP TABLE IF EXISTS playersweeks")
  dbSendQuery(con, "DROP TABLE IF EXISTS serversweeks")
    ...
    dbSendQuery(con, paste0("CREATE TABLE serversweeks AS (", server_by_week_query,")"))
    dbSendQuery(con, "ALTER TABLE serversweeks ADD UNIQUE (server, year, week)")
    ...
    dbSendQuery(con, "ALTER TABLE serversweeks ADD COLUMN latency20ppl FLOAT DEFAULT NULL")
    dbSendQuery(con, paste0("WITH latencies AS 
    ...
    dbSendQuery(con, "ALTER TABLE serversweeks ADD COLUMN latency10ppl FLOAT DEFAULT NULL")
    dbSendQuery(con, paste0("WITH latencies AS 
    ...
    dbSendQuery(con, "ALTER TABLE serversweeks ADD COLUMN latency50pct FLOAT DEFAULT NULL")
    dbSendQuery(con, paste0("WITH latencies AS 
    ...
    dbGetQuery(con, paste0(player_by_week_query, " ", "LIMIT 10"))
    dbSendQuery(con, paste0("CREATE TABLE playersweeks AS (", player_by_week_query,")"))
    dbSendQuery(con, "ALTER TABLE playersweeks ADD UNIQUE (server, uid, year, week)")
    ...
    dbSendQuery(con, "ALTER TABLE serversweeks ADD COLUMN nvisitsobs INTEGER NOT NULL DEFAULT 0, ADD COLUMN nuvisits INTEGER NOT NULL DEFAULT 0, ADD COLUMN genivisits FLOAT DEFAULT NULL ")
...
if (remake_visits_data) {
    ...
    pvisits <- data.table(dbGetQuery(con, "SELECT server, uid, year, week FROM playersweeks"))
    ...
    dbSendQuery(con, "ALTER TABLE serversweeks DROP COLUMN IF EXISTS ncomm4visits")
    dbSendQuery(con, "ALTER TABLE serversweeks DROP COLUMN IF EXISTS ncomm30visits")
    dbSendQuery(con, "ALTER TABLE serversweeks DROP COLUMN IF EXISTS bestweek4visits")
    dbSendQuery(con, "ALTER TABLE serversweeks DROP COLUMN IF EXISTS bestweek30visits")
    dbSendQuery(con, "ALTER TABLE serversweeks DROP COLUMN IF EXISTS bghost")
    dbSendQuery(con, "DROP TABLE IF EXISTS servers")


### number of players who came mor than thirty times ina  month
dbSendQuery(con, "ALTER TABLE serversweeks ADD COLUMN ncomm30visits INTEGER NOT NULL DEFAULT 0, ADD COLUMN bghost BOOLEAN DEFAULT TRUE")
dbSendQuery(con, paste0("WITH monthvisits AS 
...
dbWriteTable(con, "serversweeks_community", asdf(pvisits[,.(server, year, week, regulars)]))
dbSendQuery(con, "ALTER TABLE serversweeks ADD COLUMN ncomm4visits INTEGER DEFAULT NULL")
dbSendQuery(con, paste0(" UPDATE serversweeks AS sw SET (ncomm4visits) = (community.regulars) FROM serversweeks_community AS community
                         WHERE sw.server = community.server AND sw.year = community.year AND sw.week = community.week"))
dbSendQuery(con, "DROP TABLE IF EXISTS serversweeks_community")


### find the best week for each metric
dbSendQuery(con, "ALTER TABLE serversweeks ADD COLUMN bestweek4visits BOOLEAN DEFAULT NULL")
dbSendQuery(con, paste0("WITH bestweek AS (SELECT DISTINCT ON (server) *, '4regvisits'::VARCHAR AS besttype, ncommmax = ncomm4visits AS bestweek 
...
dbSendQuery(con, "ALTER TABLE serversweeks ADD COLUMN bestweek30visits BOOLEAN DEFAULT FALSE")
dbSendQuery(con, paste0("WITH bestweek AS (SELECT DISTINCT ON (server) *, '30visits'::VARCHAR AS besttype, ncommmax = ncomm30visits AS bestweek 
...
dbSendQuery(con, paste0("CREATE TABLE servers AS SELECT DISTINCT ON (server) *
                        FROM ( SELECT *, MAX(ncomm4visits) OVER (PARTITION BY server ORDER BY year, week) AS ncommmax FROM serversweeks
                        ) AS inner1 WHERE ncommmax = ncomm4visits OR ncommmax IS NULL ORDER BY server, RANDOM()"))
