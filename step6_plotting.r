...
mc_sub <- filterDataSetDown(mc, cutUnrealistic=TRUE, cutNonVanilla=TRUE, cutNonPositiveDependent=TRUE, featureCountMin=15, keepFeatTypes=c('plugin', 'tag', 'keyword', 'property'), keepDataSource=c('reddit', 'omni', 'mcs_org'))

mc_sub <- mc_sub[,c(lapply(.SD[,.(srv_details, dataset_source, weeks_up_todate, date_ping_int, srv_max, genivisits, ncomm4visits, latency10ppl, latency20ppl, latency50pct, T1, T2, T3, T4, srv_repstat, srv_repquery, srv_repplug, srv_repsample, srv_repsniff, srv_reptopic, nuvisits, nvisitsobs, nvisitsunobs)], unique), lapply(.SD[,.(action_other_up, action_other_down, action_admin, role_rank, role_type, boundary, economy, communication, rbasic,rbiophys, popvol, popforce, rprevent, rexternal, rhetero, rincntv, rinst, rtech, rspat, rsoccap, rleader, rnorms)], sum), infopred_central=sum(rinfopred==1), infopred_dist=sum(rinfopred==2), strategy=sum(inst_type=="Strategy"), norm=sum(inst_type=="Norm"), rule=sum(inst_type=="Rule"), nodiscretion=sum(inst_type=="No discretion")), by=.(srv_addr)]
...
summary(lm(srv_max ~        weeks_up_todate + date_point_int + rbiophys + popforce + rprevent + rexternal + rhetero + rinst + rtech + rspat + rleader + infopred_central + infopred_dist + rnorms, mc_sub))
#summary(lm(ncomm30visits ~     weeks_up_todate + date_point_int +    srv_max*rbiophys + srv_max*popforce + srv_max*rprevent + srv_max*rexternal + srv_max*rhetero + srv_max*rinst + srv_max*rtech + srv_max*rspat + srv_max*rleader + srv_max*infopred_central + srv_max*infopred_dist + srv_max*rnorms, mc_sub))
summary(lm(ncomm4visits ~        weeks_up_todate + date_point_int + srv_max*rbiophys + srv_max*popforce + srv_max*rprevent + srv_max*rexternal + srv_max*rhetero + srv_max*rinst + srv_max*rtech + srv_max*rspat + srv_max*rleader + srv_max*infopred_central + srv_max*infopred_dist + srv_max*rnorms, mc_sub))
...
#summary(rlm(ncomm30visits ~     weeks_up_todate + date_point_int +    srv_max*rbiophys + srv_max*popforce + srv_max*rprevent + srv_max*rexternal + srv_max*rhetero + srv_max*rinst + srv_max*rtech + srv_max*rspat + srv_max*rleader + srv_max*infopred_central + srv_max*infopred_dist + srv_max*rnorms, mc_sub))
summary(rlm(ncomm4visits ~        weeks_up_todate + date_point_int + srv_max*rbiophys + srv_max*popforce + srv_max*rprevent + srv_max*rexternal + srv_max*rhetero + srv_max*rinst + srv_max*rtech + srv_max*rspat + srv_max*rleader + srv_max*infopred_central + srv_max*infopred_dist + srv_max*rnorms, mc_sub))
#summary(lm(ncomm30visits ~     weeks_up_todate + date_point_int +    srv_max*rbiophys + srv_max*popforce + srv_max*rprevent + srv_max*rexternal + srv_max*rhetero + srv_max*rinst + srv_max*rtech + srv_max*rspat + srv_max*rleader + srv_max*infopred_central + srv_max*infopred_dist + srv_max*rnorms, mc_sub[dataset_source %in% c('reddit', 'mcs_org')]))
summary(lm(ncomm4visits ~        weeks_up_todate + date_point_int  + srv_max*rbiophys + srv_max*popforce + srv_max*rprevent + srv_max*rexternal + srv_max*rhetero + srv_max*rinst + srv_max*rtech + srv_max*rspat + srv_max*rleader + srv_max*infopred_central + srv_max*infopred_dist + srv_max*rnorms, mc_sub[dataset_source %in% c('reddit', 'mcs_org')]))
